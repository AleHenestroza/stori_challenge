...
> [!WARNING]
> In some cases, if a postgresql image was already donwloaded and run, there may be a data volume that may interfere with the initialization of the database. In order to fix this issue, you can run `docker-compose down -v` and then `docker-compose up`
